...
#include <Zanix/Maths/Vector3.hpp>
#include <Zanix/Maths/Vector4.hpp>

namespace Zx
{
	/*
	@brief : Constructs a Matrix4 with the param components
	@param : The components of the Matrix4
	*/
	template <typename T>
	Matrix4<T>::Matrix4(T newC11 = 1, T newC12 = 0, T newC13 = 0, T newC14 = 0,
	...
	/*
	@brief : Sets this Matrix4 to the identity
	*/
	template <typename T>
	...
	/*
	@brief : Returns the determinant of this Matrix4
	*/
	template <typename T>
	...
	/*
	@brief : Returns the transposed Matrix4 of this Matrix4
	*/
	template <typename T>
	Matrix4<T> Matrix4<T>::GetTranspose() const
	{
		return Matrix4<T>(c11, c21, c31, c41,
						c12, c22, c32, c42,
						c13, c23, c33, c43,
						c14, c24, c34, c44);
	}
	...
	/*
	@brief : Returns the inversed Matrix4 of this Matrix4
	*/
	template <typename T>
	...
	/*
	@brief : Sets a translation Matrix4
	@param : A constant reference to the Vector3 represent the translation components
	*/
	template <typename T>
	void Matrix4<T>::SetTranslationMatrix(const Vector3<T>& vec)
	{
	...
	@param : A constant reference to the Vector3 represent the scaling components
	*/
	template <typename T>
	void Matrix4<T>::SetScalingMatrix(const Vector3<T>& vec)
	{
	...
	*/
	template <typename T>
	void Matrix4<T>::SetRotationX(T angle)
	{
		T cos = std::cos(angle);
		T sin = std::sin(angle);
	...
	*/
	template <typename T>
	void Matrix4<T>::SetRotationY(T angle)
	{
		T cos = std::cos(angle);
		T sin = std::sin(angle);
	...
	*/
	template <typename T>
	void Matrix4<T>::SetRotationZ(T angle)
	{
		T cos = std::cos(angle);
		T cos = std::sin(angle);
	...
	/*
	@brief : Returns a Vector3 of the translation of this Matrix4
	*/
	template <typename T>
	Vector3<T> Matrix4<T>::GetTranslation() const
	{
		return Vector3<T>(c14, c24, c34);
	}

	/*
	@brief : Returns a ZVector who is the result of the transform of a Vector3
	@param : A constant reference to the Vector3
	@param : The w component
	*/
	template <typename T>
	Vector3<T> Matrix4<T>::Transform(const Vector3<T>& vec, T w) const
	{
		return Vector3<T>(vec.x * c11 + vec.y * c21 + vec.z * c31 + w * c41,
						   vec.x * c12 + vec.y * c22 + vec.z * c32 + w * c42,
						   vec.x * c13 + vec.y * c23 + vec.z * c33 + w * c43);
	}

	/*
	@brief : Returns a ZVector who is the result of the transform of a Vector4
	@param : A constant reference to the Vector4
	*/
	template <typename T>
	Vector4<T> Matrix4<T>::Transform(const Vector4<T>& vec) const
	{
		return Vector4<T>(vec.x * c11 + vec.y * c21 + vec.z * c31 + vec.w * c41,
	...
	/*
	@brief : Sets this Matrix4 orthogonal
	...
	*/
	template <typename T>
	void Matrix4<T>::SetOrtho(T left, T top, T right, T bottom)
	{
	...
	*/
	...
	/*
	@brief : Returns the negate of this Matrix4
	*/
	template <typename T>
	Matrix4<T> Matrix4<T>::operator-() const
	{
		return Matrix4<T>(-c11, -c12, -c13, -c14,
						   -c21, -c22, -c23, -c24,
						   -c31, -c32, -c33, -c34,
					       -c41, -c42, -c43, -c44);
	}
	...
	/*
	@brief : Returns the result of the addition of this Matrix4 and another Matrix4
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	Matrix4<T> Matrix4<T>::operator+(const Matrix4<T>& m) const
	{
		return Matrix4<T>(c11 + m.c11, c12 + m.c12, c13 + m.c13, c14 + m.c14,
	...
	/*
	@brief : Returns the result of the substract of this Matrix4 and another Matrix4
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	Matrix4<T> Matrix4<T>::operator-(const Matrix4<T>& m) const
	...
	/*
	@brief : Adds to this Matrix4 another Matrix4 (macro +=)
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	...
	/*
	@brief : Substracts to this Matrix4 another Matrix4 (macro -=)
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	const Matrix4<T>& Matrix4<T>::operator-=(const Matrix4<T>& m)
	...
	/*
	@brief : Returns the result of the multiplication of this Matrix4 and another Matrix4
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	Matrix4<T> Matrix4<T>::operator*(const Matrix4<T>& m) const
	{
		return Matrix4<T>(c11 * m.c11 + c21 * m.c12 + c31 * m.c13 + c41 * m.c14,
	...
	/*
	@brief : Returns the result of the multiplication of this Matrix4 and another Matrix4 (macro *=)
	@param : A constant reference to the other Matrix4
	*/
	template <typename T>
	const Matrix4<T>& Matrix4<T>::operator*=(const Matrix4<T>& m)
	...
	/*
	@brief : Returns the result of the multiplication of this Matrix4 and a scale (macro *=)
	@param : The scale
	*/
	template <typename T>
	const Matrix4<T>& Matrix4<T>::operator*=(T scale)
	{
	...
	/*
	@brief : Returns the result of the division of this Matrix4 and a scale (macro /=)
	@param : The scale
	*/
	template <typename T>
	const Matrix4<T>& Matrix4<T>::operator/=(T scale)
	...
	/*
	@brief : Compares two Matrix4, return true if they are equal, false otherwise
	@param : The Matrix4 to compare
	*/
	template <typename T>
	...
	/*
	@brief : Compares two Matrix4, return true if they are not equal, false otherwise
	@param : The Matrix4 to compare
	*/
	template <typename T>
	bool Matrix4<T>::operator!=(const Matrix4<T>& m) const
	...
	*/
	template <typename T>
	T Matrix4<T>::operator ()(std::size_t i, std::size_t j) const
	{
		return (&c11)[j * 4 + i];
	}

	/*
	@brief : Returns a constant pointer to the Matrix4 components
	*/
	template <typename T>
	...
	/*
	@brief : Returns a pointer to the Matrix4 components
	*/
	template <typename T>
...
/*
@brief : Displays on the output stream a Matrix4
@param : The output stream
@param : The Matrix4 to display
*/
template <typename T>
std::ostream& operator<<(std::ostream& stream, const Zx::Matrix4<T>& m)
{
